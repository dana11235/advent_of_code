...
# This won't work for the sample input, but will work for the actual input due to special properties of the input. I guessed this solution after
# realizing that any legitimate solution wouldn't run in a reaonable amount of time (I coded two brute force solutions in 12a-1 and 12a-2).
fit = 0
for box in BOXES:
    dimensions = (box["width"], box["length"])
    if box["width"] * box["length"] >= 9 * sum(box["contents"]):
        print("matched")
        fit += 1
    else:
        print("no match")
