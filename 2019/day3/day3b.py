...
with open('day3_input.txt', 'r') as file:
    for line in file:
        ...
        for move in moves:
            ...
            move_coords.append(
                {'dir': direction, 'from': coordinates, 'to': new_coordinates, 'dist': dist})
            dist += abs(new_coordinates[1] - coordinates[1]) + \
                abs(new_coordinates[0] - coordinates[0])
    ...
    for j in range(len(WIRE_COORDS[1])):
        ...
        if first_segment['dir'] in ['U', 'D'] and second_segment['dir'] in ['L', 'R'] and first_segment['from'] != [0, 0]:
            ...
            if (first_x >= second_x[0] and first_x <= second_x[1]) and (second_y >= first_y[0] and second_y <= first_y[1]):
                ...
                dist_1 = first_segment['dist'] + \
                    abs(intersection[0] - first_segment['from'][0]) + \
                    abs(intersection[1] - first_segment['from'][1])
                dist_2 = second_segment['dist'] + \
                    abs(intersection[0] - second_segment['from'][0]) + \
                    abs(intersection[1] - second_segment['from'][1])
                dist = dist_1 + dist_2
