...
heapq.heappush(MOVES, (0, 0, START.copy()))
...
while len(MOVES) > 0:
    num_moves, _, next = heapq.heappop(MOVES)
    if next == END:
        print('part a', num_moves)
        break

    key = str(next)
    if key in REVERSE_PORTALS:
    ...
    if key not in VISITED:
        ...
        for dir in DIRECTIONS:
            ...
            if in_bounds(new_move[0], new_move[1]) and MAZE[new_move[0]][new_move[1]] == '.':
                heapq.heappush(MOVES, (num_moves + 1, random.random(), new_move))